...
exports.summarizeBodhiMemory = exports.extractActionItems = exports.onMessageCreated = void 0;
...
function otherUid(chatId, myUid) {
...
// ─────────────────────────────────────────────────────────────────────────────
exports.onMessageCreated = functions.firestore.onDocumentCreated("onesutra_chats/{chatId}/messages/{msgId}", async (event) => {
    ...
    const approxNewUnread = (Number(chatData.unreadCounts?.[recipientId]) || 0) + 1;
    await chatRef.set({
        lastMessage: {
            text: msg.text ?? "",
            senderId,
    ...
    if (approxNewUnread > 2) {
        try {
            ...
            const tatvResult = await tatvModel.generateContent(`Summarize this short chat thread in MAXIMUM 6 words. Sound natural and informative. Do not use quotes.\n\nThread:\n${thread}`);
    ...
    if (!recipientData.isAutoPilotEnabled)
        return;
...
// ─────────────────────────────────────────────────────────────────────────────
exports.extractActionItems = functions.firestore.onDocumentUpdated("onesutra_chats/{chatId}", async (event) => {
                    ...
                    tasks: {
                        ...
                        items: {
                            ...
                            properties: {
                                ...
                                who: { type: generative_ai_1.SchemaType.STRING },
                                what: { type: generative_ai_1.SchemaType.STRING },
                                deadline: { type: generative_ai_1.SchemaType.STRING, description: "ISO date or 'unspecified'" },
                                status: { type: generative_ai_1.SchemaType.STRING, enum: ["pending", "done"] },
                            },
...
exports.summarizeBodhiMemory = functions.firestore.onDocumentCreated("users/{userId}/short_term_messages/{msgId}", async (event) => {
