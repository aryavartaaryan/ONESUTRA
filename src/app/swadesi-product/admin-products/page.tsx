...
import { getMockProducts, getMockSellerApps, getTotalAppUsersCount } from "@/lib/mockStore";
...
import AdminLiveDataSections from "@/components/AdminLiveDataSections";
...
export default async function AdminProductsPage() {
  ...
  const totalAppUsers = getTotalAppUsersCount();
    ...
    <div className="page-container" style={{ padding: "2rem 1rem", color: "var(--text-main)" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2.5rem", borderBottom: "1px solid rgba(255,255,255,0.1)", paddingBottom: "1.5rem" }}>
        <div>
          ...
          <p style={{ color: "var(--text-muted)", fontSize: "1.1rem" }}>Welcome, {(session?.user as any)?.name || "Admin"}</p>
        </div>
      </div>
      ...
      {/* === LIVE DATA: Donations, Pro Members, Feedback === */}
      {/* Rendered client-side for real-time Firestore updates */}
      <AdminLiveDataSections />

      {/* Products Table */}
      <div style={{ marginTop: "3rem" }}>
        <h2 style={{ fontSize: "2rem", fontWeight: "bold", color: "var(--accent-gold)", marginBottom: "1.5rem" }}>🛒 All Products</h2>
